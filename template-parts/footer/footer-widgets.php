...
if ( ! apply_filters( 'dimax_footer_widget_section', true ) ) {
	return;
}

$add_class = Helper::get_option( 'footer_widget_border' ) ? 'has-divider' : '';
$add_class .= ! intval( Helper::get_option( 'mobile_footer_widget' ) ) ? 'dimax-hide-on-mobile' : '';

?>

<div class="footer-widgets widgets-area <?php echo esc_attr( $add_class ) ?>">
	<div class="footer-container <?php echo esc_attr( apply_filters( 'dimax_footer_container_class', Helper::get_option( 'footer_container' ), 'widgets' ) ); ?>">
		<div class="row-flex">
			<?php
			if($layout  != '5-columns'){
				for ( $i = 1; $i <= $columns; $i++ ) {
