<?php
/**
 * Dimax helper functions and definitions.
 *
 * @package Dimax
 */
...
class Helper {
	...
	public static function get_fonts_url() {
		...
		*/
		if ( 'off' !== _x( 'on', 'Jost font: on or off', 'dimax' ) ) {
	...
	public static function posts_found() {
		...
		if ( $wp_query && isset( $wp_query->found_posts ) ) {
			...
			$post_text = $wp_query->found_posts > 1 ? esc_html__( 'posts', 'dimax' ) : esc_html__( 'post', 'dimax' );

			if ( \Dimax\Helper::is_catalog() ) {
				$post_text = $wp_query->found_posts > 1 ? esc_html__( 'products', 'dimax' ) : esc_html__( 'product', 'dimax' );
			}
			...
			echo sprintf( '<div class="dimax-posts__found"><div class="dimax-posts__found-inner">%s<span class="current-post"> %s </span> %s <span class="found-post"> %s </span> %s <span class="count-bar"></span></div> </div>',
				esc_html__( 'Showing', 'dimax' ), $wp_query->post_count, esc_html__( 'of', 'dimax' ), $wp_query->found_posts, $post_text );
	...
	public function load_pagination() {
		...
		$more_text = ! empty( Helper::get_option( 'blog_view_more_text' ) ) ? Helper::get_option( 'blog_view_more_text' ) : esc_html__('Load More', 'dimax');
        ...
        <nav class="next-posts-navigation navigation load-navigation">
            <div class="nav-links">
				<?php if ( get_next_posts_link() ) : ?>
                    <div id="dimax-blog-previous-ajax" class="nav-previous-ajax">
	...
	public static function get_content_limit( $num_words, $more = "&hellip;", $content = '' ) {
		if ( class_exists( '\Dimax\Addons\Helper' ) && method_exists( '\Dimax\Addons\Helper', 'get_content_limit' ) ) {
			return \Dimax\Addons\Helper::get_content_limit( $num_words, $more, $content );
		}
	...
	public static function currency_switcher( $args = array() ) {
		...
		?>
		<div class="dimax-currency list-dropdown <?php echo esc_attr( $args['direction'] ) ?>">
			...
			<div class="dropdown">
				<span class="current">
					...
					<?php echo \Dimax\Icon::get_svg( 'chevron-bottom' ) ?>
				</span>
	...
	public static function language_switcher( $args = array() ) {
		...
		<div class="dimax-language list-dropdown <?php echo esc_attr( $args['direction'] ) ?>">
			<?php if ( ! empty( $args['label'] ) ) : ?>
				<span class="label"><?php echo esc_html( $args['label'] ); ?></span>
			<?php endif; ?>
			<div class="dropdown">
	...
	public static function socials_menu() {
		if ( has_nav_menu( 'socials' ) ) {
			if ( class_exists( '\Dimax\Addons\Modules\Mega_Menu\Socials_Walker' ) ) {
				wp_nav_menu( apply_filters( 'dimax_navigation_social_content', array(
					...
					'link_before'     => '<span>',
					'link_after'      => '</span>',
					'walker' 		=>	new \Dimax\Addons\Modules\Mega_Menu\Socials_Walker()
				) ) );
			} else {
				wp_nav_menu( apply_filters( 'dimax_navigation_social_content', array(
					'theme_location'  => 'socials',
	...
	public static function post_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( esc_html__( ', ', 'dimax' ) );
			if ( $categories_list ) {
				/* translators: 1: list of categories. */
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'dimax' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'dimax' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'dimax' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
			...
			comments_popup_link(
				sprintf(
					...
					/* translators: %s: post title */
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'dimax' ),
						array(
							'span' => array(
								'class' => array(),
							),
