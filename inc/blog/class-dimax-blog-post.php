...
class Post {
	...
	public function __construct() {
		...
		add_action( 'dimax_after_open_post_content', array( $this, 'open_entry_header' ), 10 );
		add_action( 'dimax_after_open_post_content', array( $this, 'get_title' ), 20 );
		add_action( 'dimax_after_open_post_content', array( $this, 'get_entry_meta' ), 30 );
		add_action( 'dimax_after_open_post_content', array( $this, 'get_post_thumbnail' ), 40 );
		add_action( 'dimax_after_open_post_content', array( $this, 'close_entry_header' ), 50 );

		add_action( 'dimax_after_open_post_content', array( $this, 'open_get_content' ), 60 );
		add_action( 'dimax_before_close_post_content', array( $this, 'close_get_content' ), 10 );

		add_action( 'dimax_before_close_post_content', array( $this, 'open_entry_footer' ), 20 );
		add_action( 'dimax_before_close_post_content', array( $this, 'meta_tag' ), 30 );
		add_action( 'dimax_before_close_post_content', array( $this, 'meta_socials' ), 40 );
		add_action( 'dimax_before_close_post_content', array( $this, 'close_entry_footer' ), 50 );
	}
	...
	public function get_post_thumbnail() {
		...
		\Dimax\Helper::post_thumbnail( 'dimax-post-full' );
	}
	...
	public function meta_author() {
		...
		/* translators: %s: post author. */
			esc_html_x( ' by %s', 'post author', 'dimax' ),
	...
	public function meta_cat() {
		...
		$i      = 0;
		$number = apply_filters( 'dimax_meta_cat_number', 0 );
		...
		if ( ! is_wp_error( $cats ) && $cats ) {
			...
			$cat_html = sprintf( '<div class="meta meta-cat"><span class="before-text">%s</span> %s</div>', esc_html__( 'in', 'dimax' ), implode( '', $output ) );
		}
	...
	public function meta_tag() {
		...
		if ( has_tag() ) :
			the_tags( '<div class="dimax-post__tag">', ' ', '</div>' );
		endif;
	}
	...
	public function meta_socials() {
		if ( ! class_exists( '\Dimax\Addons\Helper' ) && ! method_exists( '\Dimax\Addons\Helper','share_link' )) {
			return;
		}
		...
		if ( ( ! empty( $socials ) ) ) {
			...
			foreach ( $socials as $social => $value ) {
				$output[] = \Dimax\Addons\Helper::share_link( $value );
			}
			echo sprintf( '<div class="dimax-post__socials-share">%s</div>', implode( '', $output ) );
		};
	}
	...
	public function close_get_content() {
		...
		wp_link_pages( array(
			'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'dimax' ),
			'after'  => '</div>',
		) );
