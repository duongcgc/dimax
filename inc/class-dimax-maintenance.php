<?php
/**
 * Dimax maintenance functions and definitions.
 *
...
class Maintenance {
	...
	public function maintenance_redirect() {
		...
		if ( ! $page_id || ! $page_url ) {
			if ( 'coming_soon' == $mode ) {
				$message = sprintf( '<h1>%s</h1><p>%s</p>', esc_html__( 'Coming Soon', 'dimax' ), esc_html__( 'Our website is under construction. We will be here soon with our new awesome site.', 'dimax' ) );
			} else {
				$message = sprintf( '<h1>%s</h1><p>%s</p>', esc_html__( 'Website Under Maintenance', 'dimax' ), esc_html__( 'Our website is currently undergoing scheduled maintenance. Please check back soon.', 'dimax' ) );
			}
		...
		if ( ! is_page( $page_id ) ) {
			...
			add_filter( 'dimax_topbar', '__return_false' );
			add_filter( 'dimax_get_campaign_bar', '__return_false' );
			add_filter( 'dimax_get_header', '__return_false' );
			add_filter( 'dimax_get_header_mobile', '__return_false' );
			add_filter( 'dimax_get_page_header', '__return_false' );
			add_filter( 'dimax_get_footer', '__return_false' );
			add_filter( 'dimax_newsletter_popup', '__return_false' );
