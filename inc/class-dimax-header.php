...
class Header {
	...
	public function __construct() {
		...
		add_action( 'dimax_before_open_site_header', array( $this, 'show_header_sticky_minimized' ) );

		add_action( 'dimax_after_open_site_header', array( $this, 'show_header' ) );

		// Mobile
		add_action( 'dimax_after_open_site_header', array( $this, 'mobile_header' ), 99 );

		add_filter( 'dimax_header_mobile_class', array( $this, 'mobile_header_classes' ) );

		add_action( 'dimax_header_mobile_content', array( $this, 'mobile_header_left' ), 10 );
		add_action( 'dimax_header_mobile_content', array( $this, 'mobile_header_logo' ), 30 );
		add_action( 'dimax_header_mobile_content', array( $this, 'mobile_header_icons' ), 50 );
	...
	public function enqueue_scripts() {
		wp_register_style( 'dimax-fonts', Helper::get_fonts_url(), array(), '20200928' );

		$style_file = is_rtl() ? 'style-rtl.css' : 'style.css';
		wp_enqueue_style( 'dimax', apply_filters( 'dimax_get_style_directory_uri', get_template_directory_uri() ) . '/' . $style_file, array(
			'dimax-fonts',
		), '20220217' );

		do_action( 'dimax_after_enqueue_style' );
		...
		wp_enqueue_script( 'dimax', get_template_directory_uri() . '/assets/js/scripts' . $debug . '.js', array(
			'jquery',
			'isInViewport',
			'swiper',
			'notify',
		...
		$dimax_data = array(
			...
			'nonce'                => wp_create_nonce( '_dimax_nonce' ),
		...
		$dimax_data = apply_filters( 'dimax_wp_script_data', $dimax_data );

		wp_localize_script(
			'dimax', 'dimaxData', $dimax_data
		);
	...
	public function show_header_sticky_minimized() {
		...
		if ( ! apply_filters( 'dimax_get_header', $show_header ) ) {
			return;
		}
	...
	protected function get_header_contents( $sections, $options, $atts = array() ) {
        ...
        <div class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>" <?php echo esc_attr( $attr ); ?>>
            <div class="dimax-header-container <?php echo esc_attr( apply_filters( 'dimax_header_container_class', $container_width ) ); ?>">
	...
	protected function get_header_items( $items, $options ) {
		...
		foreach ( $items as $item ) {
			...
			switch ( $item['item'] ) {
				case 'hamburger':
					\Dimax\Theme::instance()->set_prop( 'modals', $item['item'] );
					break;
	...
	public function primary_menu( $mega_menu = true ) {
		...
		$primary_menu = get_post_meta( \Dimax\Helper::get_post_ID(), 'rz_header_primary_menu', true );
		...
		if ( $mega_menu == true && Helper::get_header_layout() != 'v6' && class_exists( '\Dimax\Addons\Modules\Mega_Menu\Walker' ) ) {
			wp_nav_menu( apply_filters( 'dimax_navigation_primary_content', array(
	...
	public static function classes( $classes ) {
		...
		if ( ! get_post_meta( \Dimax\Helper::get_post_ID(), 'rz_hide_header_border', true ) ) {
			$classes .= ' site-header__border';
		}

		echo esc_attr( apply_filters( 'dimax_site_header_class', $classes ) );
	}
	...
	public function mobile_header() {
		if ( ! apply_filters( 'dimax_get_header_mobile', true ) ) {
			return;
		}
	...
	public function mobile_header_icons() {
		...
		foreach ( $icons as $icon ) {
			...
			switch ( $icon['item'] ) {
				case 'cart':
					\Dimax\Theme::instance()->set_prop( 'modals', 'cart' );
				...
				case 'account':
					\Dimax\Theme::instance()->set_prop( 'modals', 'account' );
				...
				case 'search':
					\Dimax\Theme::instance()->set_prop( 'modals', 'search' );
				...
				default:
					do_action( 'dimax_mobile_header_icon', $icon['item'] );
					break;
			}
		}
            ...
            <div class="menu-mobile-panel-content panel-content">
                ...
                <div class="modal-content">
                    <nav class="hamburger-navigation menu-mobile-navigation">
						...
						if ( class_exists( '\Dimax\Addons\Modules\Mega_Menu\Mobile_Walker' ) ) {
							$arg['walker'] = new \Dimax\Addons\Modules\Mega_Menu\Mobile_Walker();
						}
                    ...
                    <div class="content-footer">
						...
						if ( intval( Helper::get_option( 'mobile_menu_show_socials' ) ) ) {
							if ( has_nav_menu( 'socials' ) ) {
								...
								if ( class_exists( '\Dimax\Addons\Modules\Mega_Menu\Socials_Walker' ) ) {
									$args['walker'] = new \Dimax\Addons\Modules\Mega_Menu\Socials_Walker();
								}
	...
	public static function search_quicklinks( $post_type = 'product' ) {
		...
		<div class="quick-links">
			<p class="label"><?php esc_html_e( 'Quick Links', 'dimax' ); ?></p>
