...
class Layout {
	...
	public function __construct() {
		...
		add_action( 'dimax_after_open_site_content', array( $this, 'open_site_content_container' ) );

		add_action( 'dimax_before_close_site_content', array( $this, 'close_site_content_container' ) );

		add_filter( 'dimax_get_sidebar', array( $this, 'has_sidebar' ) );

		add_filter('dimax_site_content_class', array( $this, 'site_content_class' ));
	...
	public function body_classes( $classes ) {
		...
		$get_id = \Dimax\Helper::get_post_ID();
		$background = get_post_meta( $get_id, 'rz_header_background', true );
		$background = apply_filters('dimax_get_header_background', $background);
	...
	public function content_layout() {
		...
		} else if ( \Dimax\Helper::is_blog() ) {
			if ( ! is_active_sidebar( 'blog-sidebar' ) ) {
				$layout = 'full-content';
			} else {
				$layout = Helper::get_option( 'blog_layout' );
			}
		} elseif ( \Dimax\Helper::is_catalog() && \Dimax\WooCommerce\Helper::get_catalog_layout() == 'grid' ) {
		...
		return apply_filters( 'dimax_site_layout', $layout );
	}
	...
	public function site_content_class($classes) {
		...
		if( is_page() || \Dimax\Helper::is_catalog() ) {
