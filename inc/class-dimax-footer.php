...
class Footer {
	...
	public function __construct() {
		add_action( 'dimax_after_open_site_footer', array( $this, 'show_footer' ) );
		add_action( 'dimax_after_close_site_footer', array( $this, 'gotop_button' ) );
	...
	public function show_footer() {
		$show_footer = ! get_post_meta( \Dimax\Helper::get_post_ID(), 'rz_hide_footer_section', true );
		if ( ! apply_filters( 'dimax_get_footer', $show_footer ) ) {
			return;
		}

		$sections = apply_filters( 'dimax_get_footer_sections', Helper::get_option( 'footer_sections' ) );
	...
	public static function classes( $classes ) {
		...
		$footer_border_custom = get_post_meta( \Dimax\Helper::get_post_ID(), 'rz_footer_section_border_top', true );
		...
		echo apply_filters( 'dimax_site_footer_class', $classes);
	}
	...
	public function footer_item( $item ) {
		switch ( $item ) {
			...
			default:
				do_action( 'dimax_footer_footer_main_item', $item );
				break;
		}
	}
        ...
        <div class="footer-branding">
            <a href="<?php echo esc_url( home_url( '/' ) ) ?>" class="logo <?php echo esc_attr( $class ) ?>">
				<?php if ( 'svg' == $logo_type ) : ?>
					<?php echo \Dimax\Icon::sanitize_svg( $logo ); ?>
	...
	public function gotop_button() {
		if ( apply_filters( 'dimax_get_back_to_top', Helper::get_option( 'general_backtotop' ) ) ) {
			echo '<a href="#page" id="gotop">' . \Dimax\Icon::get_svg( 'arrow-right' ) . '</a>';
		}
	}
