...
class Search_Ajax {
	...
	public function __construct() {
		...
		add_action( 'wc_ajax_dimax_instance_search_form', array( $this, 'instance_search_form' ) );
	}
	...
	public function search_modal() {
		$modals = \Dimax\Theme::instance()->get_prop( 'modals' );
	...
	public function instance_search_form() {
		...
		$class_list  = 'result-list-found dimax-scrollbar';
		...
		if ( empty( $response ) ) {
			$response[]  = sprintf( '<div class="list-item"><span class="text">%s</span>%s</div>', esc_html__( 'Nothing matches your search', 'dimax' ), \Dimax\Icon::get_svg( 'smile' ) );
			$class_title = 'not-found';
			$class_list  = 'result-list-not-found';
		}

		if ( count( $response ) > intval( $_POST['ajax_search_number'] ) ) {
			$view_more = sprintf( '<div class="list-item view-more"><a class="dimax-button button-normal" href="#">%s%s</a></div>', esc_html__( 'View All', 'dimax' ), \Dimax\Icon::get_svg( 'arrow-right' ) );
		}

		if ( Helper::get_option( 'header_search_style' ) == 'icon' ) {
			$title_html = sprintf( '<h3 class="result-title %s">%s</h3>', esc_attr( $class_title ), esc_html__( 'Search Results:', 'dimax' ) );
		}
