...
namespace Dimax\WooCommerce\Modules;
...
class Notices {
    ...
    public function __construct() {
        ...
        add_action( 'wc_ajax_dimax_product_popup_recommended', array( $this, 'product_template_recommended' ) );

        add_action( 'dimax_product_popup_atc_recommendation', array(
            $this,
        ...
        <div id="rz-popup-add-to-cart" class="rz-popup-add-to-cart rz-modal woocommerce">
            ...
            <div class="modal-content container woocommerce">
                <div class="button-close">
                    <?php echo \Dimax\Icon::get_svg( 'close' ) ?>
                </div>
                <div class="product-modal-content">
                <div class="rz-product-popup-atc__notice">
                    <?php esc_html_e( 'Successfully added to your cart.', 'dimax' ) ?>
                </div>
               <div class="widget_shopping_cart_content"></div>
               <?php do_action( 'dimax_product_popup_atc_recommendation' ); ?>
                </div>
    ...
    */
    public function products_recommended_content($query_posts) {
        ...
        <div class="recommendation-heading">
            <h2 class="product-heading"> <?php echo ! empty( Helper::get_option( 'added_to_cart_notice_products_title' ) ) ? esc_html( Helper::get_option( 'added_to_cart_notice_products_title' ) ) : esc_html__( 'You may also like', 'dimax' ); ?> </h2>
            <div class="rz-swiper-buttons">
                  <?php echo \Dimax\Icon::get_svg('chevron-left', 'rz-swiper-button-prev'); ?>
                   <?php echo \Dimax\Icon::get_svg('chevron-right', 'rz-swiper-button-next'); ?>
            </div>
        </div>
    ...
    */
    public function notices_script_data( $data ) {
        if ( Helper::get_option( 'added_to_cart_notice' ) == 'simple' ) {
            $data['added_to_cart_notice'] = array(
                'added_to_cart_text'              => esc_html__( 'has been added to your cart.', 'dimax' ),
                'successfully_added_to_cart_text' => esc_html__( 'Successfully added to your cart.', 'dimax' ),
                'cart_view_text'                  => esc_html__( 'View Cart', 'dimax' ),
        ...
        if ( intval( Helper::get_option( 'added_to_wishlist_notice' ) ) && defined( 'YITH_WCWL' ) ) {
            $data['added_to_wishlist_notice'] = array(
                'added_to_wishlist_text'    => esc_html__( 'has been added to your wishlist.', 'dimax' ),
                'added_to_wishlist_texts'   => esc_html__( 'have been added to your wishlist.', 'dimax' ),
                'wishlist_view_text'        => esc_html__( 'View Wishlist', 'dimax' ),
	...
	public function widget_shopping_cart_count_notice() {
        ...
        if ( $count > 1 ) {
            echo sprintf( __('There are %s items in your cart', 'dimax'), $count );
        } else {
            echo sprintf( __('There is %s item in your cart', 'dimax'), $count );
        }
		echo '</span>';
	}
}
