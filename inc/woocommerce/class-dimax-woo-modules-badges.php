...
class Badges {
	...
	public function __construct() {
		...
		if ( Helper::get_option( 'product_catalog_badges' ) ) {
			add_action( 'dimax_product_loop_thumbnail', array( $this, 'product_badges' ), 0 );

		}
		add_action( 'dimax_product_loop_masonry_thumbnail', array( $this, 'product_badges' ), 5 );
		add_action( 'dimax_product_loop_showcase_thumbnail', array( $this, 'product_badges' ), 5 );
		...
		if ( Helper::get_option( 'single_product_badges' ) ) {
			add_action( 'dimax_before_product_gallery', array( $this, 'product_badges' ) );
		}
	}
	...
	public function get_badges() {
		...
		if ( Helper::get_option( 'shop_badge_soldout' ) && ! $product->is_in_stock() ) {
			...
			if ( ! $in_stock ) {
				$text               = !empty( Helper::get_option( 'shop_badge_soldout_text' ) ) ? Helper::get_option( 'shop_badge_soldout_text' ) : esc_html__( 'Sold Out', 'dimax' );
				$badges['sold-out'] = $badge = '<span class="sold-out woocommerce-badge"><span>' . esc_html( $text ) . '</span></span>';
			}
		} else {
			if ( Helper::get_option( 'shop_badge_new' ) && in_array( $product->get_id(), \Dimax\WooCommerce\Helper::get_new_product_ids() ) ) {
				$text          = ! empty( Helper::get_option( 'shop_badge_new_text' ) ) ? Helper::get_option( 'shop_badge_new_text' ) : esc_html__( 'New', 'dimax' );
			...
			if ( $product->is_featured() && Helper::get_option( 'shop_badge_featured' ) ) {
				$text               = !empty ( Helper::get_option( 'shop_badge_featured_text' ) ) ? Helper::get_option( 'shop_badge_featured_text' ) : esc_html__( 'Hot', 'dimax' );
		...
		$badges = apply_filters( 'dimax_product_badges', $badges, $product );
		ksort( $badges );
	...
	public function get_sale_flash( $output, $post, $product ) {
		...
		$text       = ! empty( Helper::get_option( 'shop_badge_sale_text' ) ) ? Helper::get_option( 'shop_badge_sale_text' ) : esc_html__( 'Sale', 'dimax' );
		$percentage = 0;
