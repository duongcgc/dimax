...
class Cart {
	...
	public function button_continue_shop() {
		echo sprintf(
			'<a href="%s" class="dimax-button button-light continue-button">%s%s</a>',
			esc_url( get_permalink( get_option( 'woocommerce_shop_page_id' ) ) ),
			\Dimax\Icon::get_svg( 'arrow-right' ),
			esc_html__( 'Continue Shopping', 'dimax' )
		);
	}
	...
	public function get_cross_sells_title() {
		$cross_sells_title =  ! empty( Helper::get_option( 'product_cross_sells_title' ) ) ? Helper::get_option( 'product_cross_sells_title' ) : esc_html__( 'You may also like', 'dimax' );
