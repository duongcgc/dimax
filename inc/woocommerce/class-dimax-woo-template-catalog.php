...
namespace Dimax\WooCommerce\Template;
...
class Catalog {
	...
	public function __construct() {
		...
		$toolbar_layout = apply_filters( 'dimax_get_catalog_toolbar_layout', Helper::get_option( 'catalog_toolbar_layout' ) );
		switch ( $toolbar_layout ) {
			case 'v1':
				switch ( Helper::get_option( 'catalog_toolbar_els' ) ) {
					case 'page_header':
						add_action( 'dimax_woocommerce_catalog_toolbar', array(
							$this,
				...
				add_action( 'dimax_woocommerce_catalog_toolbar', array(
					\Dimax\Theme::instance()->get( 'breadcrumbs' ),
					'breadcrumbs'
				), 15 );

				add_action( 'dimax_woocommerce_catalog_toolbar', array( $this, 'products_ordering' ), 15 );

				add_action( 'dimax_woocommerce_catalog_toolbar', array( $this, 'products_filter_sidebar' ), 25 );

				break;

			case 'v2':
				add_action( 'dimax_woocommerce_catalog_toolbar', array( $this, 'products_filter' ), 15 );

				break;

			case 'v3':
				add_action( 'dimax_woocommerce_catalog_toolbar', array( $this, 'products_tabs' ), 15 );
				add_action( 'dimax_woocommerce_catalog_toolbar', array(
					$this,
					'toolbar_right_open_wrapper'
				), 20 );
				add_action( 'dimax_woocommerce_catalog_toolbar', array( $this, 'products_filter_button' ), 25 );
				if( intval(Helper::get_option('catalog_toolbar_products_sorting')) ) {
					add_action( 'dimax_woocommerce_catalog_toolbar', array( $this, 'products_ordering' ), 35 );
				}
				add_action( 'dimax_woocommerce_catalog_toolbar', array(
					$this,
				...
				if ( 'modal' == Helper::get_option( 'catalog_toolbar_products_filter' ) ) {
					\Dimax\Theme::instance()->set_prop( 'modals', 'filter' );
				}
		...
		add_filter( 'dimax_primary_sidebar_classes', array(
			$this,
		...
		if ( \Dimax\Helper::get_option( 'taxonomy_description_position' ) == 'below' ) {
	...
	public function body_class( $classes ) {
		$classes[] = 'dimax-catalog-page';

		if( Helper::get_option('catalog_product_filter_sidebar') ) {
			$classes[] = 'dimax-filter-sidebar-off';
		}
	...
	public function scripts() {
		...
		wp_enqueue_script( 'dimax-product-catalog', get_template_directory_uri() . '/assets/js/woocommerce/product-catalog' . $debug . '.js', array(
			'dimax',
		), '20211209', true );

		$dimax_catalog_data = array(
			'filtered_price' => array(
				'min' => esc_html__( 'Min', 'dimax' ),
				'max' => esc_html__( 'Max', 'dimax' ),
			)
		);

		if ( intval( Helper::get_option( 'catalog_widget_collapse_content' ) ) && \Dimax\WooCommerce\Helper::get_catalog_layout() == 'grid' ) {
			$dimax_catalog_data['catalog_widget_collapse_content'] = 1;
		}

		if ( intval( Helper::get_option( 'catalog_filters_sidebar_collapse_content' ) ) && Helper::get_option( 'catalog_toolbar_layout' ) == 'v3' ) {
			$dimax_catalog_data['catalog_filters_sidebar_collapse_content'] = 1;
		}

		$dimax_catalog_data = apply_filters('dimax_get_catalog_localize_data', $dimax_catalog_data);

		wp_localize_script(
			'dimax-product-catalog', 'dimaxCatalogData', $dimax_catalog_data
		);
	...
	public function products_toolbar() {
        ...
        <div class="catalog-toolbar <?php echo esc_attr( $classes ); ?>">
			<?php do_action( 'dimax_woocommerce_catalog_toolbar' ); ?>
        </div>
	...
	public function pagination_args( $args ) {
		$args['prev_text'] = \Dimax\Icon::get_svg( 'caret-right' );
		$args['next_text'] = \Dimax\Icon::get_svg( 'caret-right' );
	...
	public function pagination() {
		...
		if ( 'numeric' == $nav_type ) {
			...
			if ( get_next_posts_link() ) {
				...
				$nav_html = sprintf( '<span class="button-text">%s</span>', esc_html__( 'Load More', 'dimax' ) );

				echo '<nav class="' . esc_attr( implode( ' ', $classes ) ) . '">';
				echo '<div id="dimax-catalog-previous-ajax" class="nav-previous-ajax">';
				next_posts_link( $nav_html );
				echo '<div class="dimax-gooey-loading">
	...
	public function products_top_categories() {
		...
		$thumbnail_size_cropped = apply_filters( 'dimax_top_categories_thumbnail_size', array(
			'width'  => '250',
			'height' => '250'
		) );
		$thumbnail_size         = apply_filters( 'dimax_top_categories_thumbnail_size', 'full' );
		...
		if ( $output ) {
			$columns = 5;
			if( \Dimax\WooCommerce\Helper::get_catalog_layout() == 'grid' ) {
			...
			$columns = apply_filters( 'dimax_top_categories_columns', $columns );

			$class = intval( Helper::get_option( 'top_categories_ajax_filter' ) ) ? 'ajax-filter' : '';
			printf(
				'<div id="rz-catalog-top-categories" class="rz-catalog-categories dimax-swiper-carousel-elementor navigation-arrows navigation-tablet-dots navigation-mobile-dots %s" data-columns="%s"><div class="swiper-container"><ul class="catalog-categories__wrapper swiper-wrapper">%s</ul></div></div>',
				esc_attr( $class ),
				esc_attr( $columns ),
				implode( ' ', $output )
			);
	...
	public function products_filter_modal() {
		if ( ! in_array( 'filter', \Dimax\Theme::instance()->get_prop( 'modals' ) ) ) {
			return;
		}
		...
		$classes = apply_filters( 'dimax_get_product_filters_modal_classes', $classes );
		$sidebar = apply_filters( 'dimax_get_product_filters_modal_sidebar', 'catalog-filters-sidebar' );
        ...
        <div id="catalog-filters-modal" class="catalog-filters-modal rz-modal" tabindex="-1" role="dialog">
            ...
            <div class="filters-panel-content panel-content">
                <div class="modal-header">
                    <h3 class="modal-title"><?php esc_html_e( 'Filter By', 'dimax' ) ?></h3>
                    <a href="#"
                           class="close-account-panel button-close"><?php echo \Dimax\Icon::get_svg( 'close' ); ?></a>
                </div>
                <div class="modal-content dimax-scrollbar catalog-sidebar <?php echo esc_attr( $classes ); ?>">
	...
	public function products_tabs() {
		...
		$base_url = \Dimax\WooCommerce\Helper::get_page_base_url();
		...
		array_unshift( $tabs, sprintf(
			...
			esc_url( $btn_url ),
			$active ? '' : 'active',
			esc_html__( 'All', 'dimax' )
		) );

		$text_toggle = $type == 'group' ? esc_html__( 'Products', 'dimax' ) : esc_html__( 'Categories', 'dimax' );

		echo '<div class="catalog-toolbar-tabs__title">' . $text_toggle . \Dimax\Icon::get_svg( 'chevron-bottom' ) . '</div>';
	...
	public function products_filter_button() {
        ...
        <a href="#catalog-filters" class="toggle-filters catalog-toolbar-item__control"
           ...
           data-target="catalog-filters-<?php echo esc_attr( $open ) ?>">
			<?php echo \Dimax\Icon::get_svg( 'filter', 'svg-normal', 'shop' ) ?>
			<?php echo \Dimax\Icon::get_svg( 'close', 'svg-active' ) ?>
            <span class="text-filter"><?php esc_html_e( 'Filter', 'dimax' ) ?></span>
        </a>
		<?php
	}
	...
	public function products_ordering() {
		$catalog_orderby_options = apply_filters( 'dimax_products_filter_order_by', array(
			'menu_order' => __( 'Default sorting', 'dimax' ),
			'popularity' => __( 'Sort by popularity', 'dimax' ),
			'rating'     => __( 'Sort by average rating', 'dimax' ),
			'date'       => __( 'Sort by latest', 'dimax' ),
			'price'      => __( 'Sort by price: low to high', 'dimax' ),
			'price-desc' => __( 'Sort by price: high to low', 'dimax' ),
		) );

		// get form action url
		$form_action = \Dimax\WooCommerce\Helper::get_page_base_url();
        ...
        <div class="woocommerce-ordering">
            <span class="woocommerce-ordering__button"><span
                        class="woocommerce-ordering__button-label"><?php echo ! empty( $orderby ) ? $order_current : esc_html__( 'Default', 'dimax' ) ?> </span> <?php echo \Dimax\Icon::get_svg( 'chevron-bottom' ); ?></span>
	...
	public function primary_sidebar_classes( $classes ) {
		...
		if ( intval( \Dimax\Helper::get_option( 'catalog_sticky_sidebar' ) ) ) {
			$classes .= ' dimax-sticky-sidebar';
		}
