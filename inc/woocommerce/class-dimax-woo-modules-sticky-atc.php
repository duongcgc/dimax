...
class Sticky_ATC {
	...
	public function __construct() {
		...
		add_filter( 'dimax_get_back_to_top', array( $this, 'get_back_to_top' ) );
	}
	...
	public function sticky_single_add_to_cart() {
		...
		$add_class = [
			'dimax-sticky-add-to-cart__content-button button dimax-button',
		...
		$sticky_class    = 'dimax-sticky-add-to-cart product-' . $product_type;
		$sticky_class    .= ' dimax-sticky-atc_' . Helper::get_option( 'product_add_to_cart_sticky_position' );
		...
		if ( $post_thumbnail_id ) {
			...
			$alt_text      = esc_html__( 'Awaiting product image', 'dimax' );
		}
		...
		?>
        <section id="dimax-sticky-add-to-cart" class="<?php echo esc_attr( $sticky_class ) ?>">
            <div class="<?php echo esc_attr($container_class); ?>">
                <div class="dimax-sticky-add-to-cart__content">
				<div class="dimax-sticky-atc__product-image"><img src="<?php echo esc_url( $thumbnail_src[0] ); ?>" alt="<?php echo esc_attr( $alt_text ); ?>" data-o_src="<?php echo esc_url( $thumbnail_src[0] );?>"></div>
                    <div class="dimax-sticky-add-to-cart__content-product-info">
                        <div class="dimax-sticky-add-to-cart__content-title"><?php the_title(); ?></div>
                        <span class="dimax-sticky-add-to-cart__content-price"><?php echo wp_kses_post( $product->get_price_html() ); ?></span>
					...
					if( function_exists('wcpa_is_wcpa_product') && wcpa_is_wcpa_product( $product->get_id() ) ) {
						echo sprintf( '<a href="%s" data-quantity="1" class="is-wcpa-product %s" data-product_id = "%s" data-text="%s" data-title="%s" rel="nofollow"> %s %s</a>',
							...
							esc_html__( 'Add to cart', 'dimax' ),
							\Dimax\WooCommerce\Helper::get_cart_icon()
						);
					} else {
        ...
        </section><!-- .dimax-sticky-add-to-cart -->
		<?php
	}
