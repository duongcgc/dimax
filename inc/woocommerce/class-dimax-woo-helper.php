...
class Helper {
	...
	public static function get_page_base_url() {
		...
		return apply_filters( 'dimax_get_page_base_url', $link );
	}
	...
	public static function get_catalog_layout() {
		$layout = \Dimax\Helper::get_option( 'shop_catalog_layout' );
		return apply_filters( 'dimax_get_catalog_layout', $layout );
	}
	...
	public static function get_product_loop_layout() {
		if ( is_null( self::$product_loop ) ) {
			$layout = \Dimax\Helper::get_option( 'product_loop_layout' );
			if ( self::get_catalog_layout() == 'masonry' && \Dimax\Helper::is_catalog() ) {
				$layout = '7';
			}

			$layout = apply_filters( 'dimax_get_product_loop_layout', $layout );
	...
	public static function get_product_video() {
		...
		if ( $video_html ) {
			...
			$video_html = '<div data-thumb="' . esc_url( $video_thumb ) . '" class="woocommerce-product-gallery__image dimax-product-video">' . $video_html . '</div>';
		}
	...
	public static function get_product_video_html() {
		...
		if ( $video_html ) {
			$video_html   = '<div class="dimax-video-wrapper ' . esc_attr( $video_class ) . '">' . $video_html . '</div>';
		}
	...
	public static function quick_view_button() {
		...
		echo sprintf(
			...
			esc_attr__( 'Quick View', 'dimax' ),
			\Dimax\Icon::get_svg( 'eye', '', 'shop' ),
			esc_html__( 'Quick View', 'dimax' )
		);
	}
	...
	public static function get_new_product_ids() {
		// Load from cache.
		$product_ids = get_transient( 'dimax_woocommerce_products_new' );
		...
		if ( \Dimax\Helper::get_option( 'shop_badge_new' ) ) {
			$newness = intval( \Dimax\Helper::get_option( 'shop_badge_newness' ) );
		...
		set_transient( 'dimax_woocommerce_products_new', $product_ids, DAY_IN_SECONDS );
	...
	public static function get_cart_icon() {
		if( \Dimax\Helper::get_option( 'cart_icon_source' ) == 'icon' ) {
			$cart_icon = \Dimax\Helper::get_option( 'cart_icon' );
			return \Dimax\Icon::get_svg( $cart_icon , 'icon-' . $cart_icon, 'shop');
		} else {
			$custom_icon  = \Dimax\Helper::get_option( 'cart_icon_svg' );
			if( $custom_icon ) {
				return '<span class="dimax-svg-icon icon-cart-custom">' . \Dimax\Icon::sanitize_svg( $custom_icon ) . '</span>';
			} else {
				return \Dimax\Icon::get_svg( 'cart', '', 'shop');
			}
		}
	}
}
