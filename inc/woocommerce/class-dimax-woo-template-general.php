...
class General {
	...
	public function __construct() {
		...
		if(\Dimax\WooCommerce\Helper::get_catalog_layout() == 'masonry') {
	...
	public function scripts() {
		...
		wp_enqueue_style( 'dimax-woocommerce-style', apply_filters( 'dimax_get_style_directory_uri', get_template_directory_uri() ) . '/' . $style_file, array(),  '20220217' );

		$parse_css = apply_filters( 'dimax_wc_inline_style', false );
		if( $parse_css ) {
			wp_add_inline_style( 'dimax-woocommerce-style', $parse_css );
		}
	...
	public function breadcrumb_args( $args ) {
		$args['delimiter']   = \Dimax\Icon::get_svg( 'chevron-right', 'delimiter' );
	...
	public function star_rating_html( $html, $rating, $count ) {
		...
		if ( 0 < $count ) {
			...
			$html .= sprintf( _n( 'Rated %1$s out of 5 based on %2$s customer rating', 'Rated %1$s out of 5 based on %2$s customer ratings', $count, 'dimax' ), '<strong class="rating">' . esc_html( $rating ) . '</strong>', '<span class="rating">' . esc_html( $count ) . '</span>' );
		} else {
			/* translators: %s: rating */
			$html .= sprintf( esc_html__( 'Rated %s out of 5', 'dimax' ), '<strong class="rating">' . esc_html( $rating ) . '</strong>' );
		}
	...
	public function get_product_availability_text( $availability, $product ) {
		...
		if ( ! $product->managing_stock() && $product->get_stock_status() == 'instock' ) {
			$availability = esc_html__( 'In stock', 'dimax' );
		}
	...
	public function product_single_wishlist() {
		...
		echo '<div class="rz-wishlist-button dimax-button button-outline ' . esc_attr( $css_class ) . '">';
	...
	public static function products_group_query( $query ) {
		...
		switch ( $_GET['products_group'] ) {
			...
			case 'new':
				$query->set( 'post__in', array_merge( array( 0 ), \Dimax\WooCommerce\Helper::get_new_product_ids() ) );
				break;
	...
	public function quantity_icon_decrease() {
		echo \Dimax\Icon::get_svg('minus', 'dimax-qty-button decrease', 'shop');
	}
	...
	public function quantity_icon_increase() {
		echo \Dimax\Icon::get_svg('plus', 'dimax-qty-button increase', 'shop');
	}
	...
	public function cart_shipping_method_full_label($label, $method) {
		...
		if( $free_cost ) {
			$label .= ':' . esc_html__(' Free Shipping', 'dimax');
		}
