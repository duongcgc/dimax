...
class Product_Loop {
	...
	public function product_loop_content_hooks() {
		...
		add_action( 'woocommerce_shop_loop_item_title', array(
			\Dimax\WooCommerce\Helper::instance(),
			'product_loop_title'
		) );
		...
		add_action( 'wc_ajax_dimax_product_loop_form', array( $this, 'product_loop_form_ajax' ) );

		add_filter( 'dimax_wp_script_data', array( $this, 'loop_script_data' ) );
	...
	public function loop_start( $html ) {
		$html            = '';
		$products_layout = \Dimax\WooCommerce\Helper::get_catalog_layout();

		$classes = array(
			'products'
		);

		$loop_layout = \Dimax\WooCommerce\Helper::get_product_loop_layout();
		...
		$classes[] = \Dimax\Helper::is_catalog() ? 'layout-' . $products_layout : '';
	...
	public function product_loop_layout() {
		...
		$featured_icons = apply_filters( 'dimax_get_product_loop_featured_icons', $featured_icons );
		$loop_layout    = \Dimax\WooCommerce\Helper::get_product_loop_layout();
		$attributes = (array) Helper::get_option( 'product_loop_attributes' );
		...
		switch ( $loop_layout ) {
			...
			case 2:
				add_action( 'dimax_product_loop_thumbnail', array(
					$this,
				...
				if ( in_array( 'cart', $featured_icons ) ) {
					add_action( 'dimax_product_loop_thumbnail', 'woocommerce_template_loop_add_to_cart', 20 );
				}
			...
			case 3:
				...
				if ( in_array( 'cart', $featured_icons ) ) {
					add_action( 'dimax_product_loop_thumbnail', 'woocommerce_template_loop_add_to_cart', 110 );
					add_action( 'woocommerce_after_shop_loop_item_title', 'woocommerce_template_loop_add_to_cart', 50 );
				}
			...
			default:
				...
				if ( in_array( 'cart', $featured_icons ) ) {
					add_action( 'dimax_product_loop_thumbnail', 'woocommerce_template_loop_add_to_cart', 10 );
				}
	...
	public function product_loop_thumbnail() {
		...
		$product_hover = apply_filters( 'dimax_get_product_loop_hover', $product_hover );
		...
		switch ( $product_hover ) {
			case 'slider':
				...
				if ( $image_ids ) {
					echo '</div>';
					echo \Dimax\Icon::get_svg( 'chevron-left', 'rz-product-loop-swiper-prev rz-swiper-button' );
					echo \Dimax\Icon::get_svg( 'chevron-right', 'rz-product-loop-swiper-next rz-swiper-button' );
				}
				do_action( 'dimax_product_loop_thumbnail' );
				echo '</div>';
				break;
			case 'fadein':
				$image_ids = $product->get_gallery_image_ids();
	...
	public function product_loop_category() {
		...
		\Dimax\WooCommerce\Helper::product_taxonomy( $taxonomy );
	}
	...
	public function product_loop_desc() {
		...
		if ( $length ) {
			$short_description = \Dimax\Helper::get_content_limit( $length, '', $short_description );
		}
	...
	public function product_loop_quick_shop() {
		...
		$icon = \Dimax\WooCommerce\Helper::get_cart_icon();

		echo sprintf(
			'<a href="#" class="product-quick-shop-button dimax-button" data-product_id="%s" >%s%s</a>',
			esc_attr( $product->get_id() ),
			$icon,
			esc_html__( 'Quick Shop', 'dimax' )
		);
	}
	...
	public function add_to_cart_link( $html, $product, $args ) {
		$newtabs = $product->is_type( 'external' ) && \Dimax\Helper::get_option( 'product_external_open' ) ? 'target="_blank"' : '';
	...
	public function product_variable_add_to_cart_text( $text ) {
		...
		if( ! empty( $product ) ) {
			...
			if ( $product->is_purchasable() ) {
				$text = esc_html__( 'Add to cart', 'dimax' );
	...
	public function loop_script_data( $data ) {
		...
		if ( in_array( \Dimax\WooCommerce\Helper::get_product_loop_layout(), array( '8', '9' ) ) ) {
			$data['product_loop_layout'] = \Dimax\WooCommerce\Helper::get_product_loop_layout();
			if ( intval( Helper::get_option( 'product_loop_variation' ) ) ) {
				$data['product_loop_variation'] = 1;
			}

			if ( intval( \Dimax\WooCommerce\Helper::get_product_loop_layout() == '9' && Helper::get_option( 'product_loop_variation_ajax' ) ) ) {
