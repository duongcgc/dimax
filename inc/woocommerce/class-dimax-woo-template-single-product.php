...
class Single_Product {
	...
	public function __construct() {
		...
		add_filter( 'dimax_wp_script_data', array(
			$this,
			'product_script_data'
		) );
		...
		switch ( $this->single_get_product_layout() ) {
			...
			case 'v6':
				add_filter( 'dimax_get_sidebar', '__return_true' );
				break;
		}
		...
		if ( Helper::get_option( 'product_play_video' ) == 'popup' ) {
			add_action( 'dimax_before_product_gallery', array( $this, 'product_video' ) );
		}
	...
	public function scripts() {
		...
		if( \Dimax\Helper::get_option('product_play_video') == 'popup' ) {
		...
		wp_enqueue_script( 'dimax-single-product', get_template_directory_uri() . '/assets/js/woocommerce/single-product' . $debug . '.js', array(
			'dimax',
		), '20211209', true );
	}
	...
	public function product_class( $classes ) {
		...
		$classes[] = Helper::get_option('product_play_video') == 'popup' ? 'dimax-play-video--popup' : '';

		$classes[] = ! intval( Helper::get_option( 'product_image_lightbox' ) ) ? 'dimax-no-product-image-lightbox' : '';
	...
	public function open_gallery_summary_wrapper() {
		$container = apply_filters( 'dimax_single_product_container_class', '' );
	...
	public function product_share() {
        ...
        <div class="product-share share">
			<span class="sharing-icon">
				<?php esc_html_e( 'Share:', 'dimax' ) ?>
			</span>
            <span class="socials">
				<?php
				foreach ( $socials as $social ) {
					echo \Dimax\Addons\Helper::share_link( $social, array( 'whatsapp_number' => $whatsapp_number ) );
				}
				?>
			</span>
        </div>
		...
		if ( ! empty( $tabs ) ) :
            ...
            <div class="woocommerce-tabs <?php echo esc_attr( $tabs_class ); ?>">
				<?php foreach ( $tabs as $key => $tab ) : ?>
                    <div class="dimax-tab-wrapper">
                        <a href="#tab-<?php echo esc_attr( $key ); ?>"
                           class="dimax-accordion-title tab-title-<?php echo esc_attr( $key ); ?>">
	...
	public function single_product_taxonomy() {
		if ( $taxonomy = Helper::get_option( 'product_taxonomy' ) ) {
			...
			\Dimax\WooCommerce\Helper::product_taxonomy( $taxonomy, $show_thumbnail );
		}
	}
	...
	public function product_gallery_is_slider() {
		...
		return apply_filters( 'dimax_product_gallery_is_slider', $support );
	}
	...
	public function single_get_product_layout() {
		...
		return apply_filters( 'dimax_single_get_product_layout', $product_layout );
	}
	...
	public function product_video() {
		...
		if( ! empty( $video_url ) ) {
			echo sprintf('<a href="%s" class="dimax-product-video--icon dimax-i-video"><span>%s</span></a>', esc_url( $video_url ), esc_html__('Play Video', 'dimax') );
		}
	}
