...
class Recently_Viewed {
	...
	public function __construct() {
		...
		if ( intval( Helper::get_option( 'recently_viewed_ajax' ) ) ) {
			add_action( 'wc_ajax_dimax_get_recently_viewed', array( $this, 'do_ajax_products_content' ) );
		}

		add_action( 'dimax_before_open_site_footer', array( $this, 'products_recently_viewed_section' ) );
	}
	...
	public function products_content() {
		...
		if ( empty( $this->product_ids ) ) {
			printf(
				...
				'<li class="text-center">%s <br><a href="%s" class="dimax-button">%s</a></li>' .
				'</ul>',
				esc_html__( 'Recently Viewed Products is a function which helps you keep track of your recent viewing history.', 'dimax' ),
				esc_url( get_permalink( get_option( 'woocommerce_shop_page_id' ) ) ),
				esc_html__( 'Shop Now', 'dimax' )
			);
		} else {
	...
	public function products_recently_viewed_section() {
		...
		if ( ! is_singular( 'product' ) && ! \Dimax\Helper::is_catalog() && ! is_cart() && ! is_checkout() ) {
			return;
		}
		...
		if ( is_singular( 'product' ) && ! in_array( 'single', $display_page ) ) {
			return;
		} elseif ( \Dimax\Helper::is_catalog() && ! in_array( 'catalog', $display_page ) ) {
			return;
		...
		if ( $button_link ) {
			$button_text = Helper::get_option( 'recently_viewed_button_text' ) ? Helper::get_option( 'recently_viewed_button_text' ) : esc_html__('View All', 'dimax');
			$button_html = sprintf( '<a href="%s" class="recently-button">%s</a>', esc_url( $button_link ), esc_html( $button_text ) );
		}

		$title_text = Helper::get_option( 'recently_viewed_title' ) ? Helper::get_option( 'recently_viewed_title' ) : esc_html__('Recently Viewed', 'dimax');
		...
		?>
        <section class="dimax-history-products <?php echo esc_attr( $addClass ) ?>" id="dimax-history-products"
