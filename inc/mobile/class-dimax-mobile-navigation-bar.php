...
class Navigation_Bar {
	...
	function __construct() {
		add_filter( 'dimax_site_footer_class', array( $this, 'footer_class' ) );
		...
		add_action( 'dimax_navigation_bar_before_atc_template', array(
			$this,
			'open_wrapper_product_type_variable'
		), 10 );
		add_action( 'dimax_navigation_bar_after_atc_template', array(
			$this,
			'close_wrapper_product_type_variable'
		), 30 );

		add_action( 'dimax_navigation_bar_content_atc_template', array( $this, 'get_content_navigation' ), 30 );
	}
		...
		foreach ( $items as $item ) {
			...
			switch ( $item ) {
				case 'cart':
					...
					\Dimax\Theme::instance()->set_prop( 'modals', $item );
					break;
	...
	public function close_wrapper_product_type_variable() {
		...
		echo \Dimax\Icon::get_svg( 'close', 'close rz-navigation-bar__btn-close active' );
		\Dimax\Theme::instance()->get( 'woocommerce' )->get_template( 'product_loop' )->display_variation_dropdown();
