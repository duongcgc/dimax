...
class Breadcrumbs {
	...
	public function get_breadcrumbs( $args = '' ) {
		$args = wp_parse_args(
			$args, array(
				'separator'         => \Dimax\Icon::get_svg( 'chevron-right', 'delimiter' ),
				'home_class'        => 'home',
				'before'            => '',
				'before_item'       => '',
				'after_item'        => '',
				...
				'labels'            => array(
					...
					'archive'   => esc_html__( 'Archives', 'dimax' ),
					'blog'      => esc_html__( 'Blog', 'dimax' ),
					'search'    => esc_html__( 'Search results for', 'dimax' ),
					'not_found' => esc_html__( 'Not Found', 'dimax' ),
					'portfolio' => esc_html__( 'Portfolio', 'dimax' ),
					'author'    => esc_html__( 'Author:', 'dimax' ),
					'day'       => esc_html__( 'Daily:', 'dimax' ),
					'month'     => esc_html__( 'Monthly:', 'dimax' ),
					'year'      => esc_html__( 'Yearly:', 'dimax' ),
				),
			)
		);

		$args = apply_filters( 'dimax_breadcrumbs_args', $args );
		...
		if ( ! $args['home_class'] ) {
			...
			$items[] = sprintf(
				...
				$args['before_item'],
				$args['home_class'],
				apply_filters( 'dimax_breadcrumbs_home_url', get_home_url() ),
				$args['labels']['home'],
				$args['after_item']
			);
		...
		elseif ( is_home() && ! is_front_page() ) {
			...
			if ( get_option( 'dimax_portfolio_page_id' ) ) {
				$title = get_the_title( get_option( 'dimax_portfolio_page_id' ) );
			} else {
				$title = esc_html__( 'Portfolio', 'dimax' );
			}
		...
		elseif ( is_day() ) {
			$items[] = sprintf(
				$item_text_tpl,
				sprintf( esc_html__( '%s %s', 'dimax' ), $args['labels']['day'], get_the_date() )
			);
		} // Month archive
		elseif ( is_month() ) {
			$items[] = sprintf(
				$item_text_tpl,
				sprintf( esc_html__( '%s %s', 'dimax' ), $args['labels']['month'], get_the_date( 'F Y' ) )
			);
		} // Year archive
		elseif ( is_year() ) {
			$items[] = sprintf(
				$item_text_tpl,
				sprintf( esc_html__( '%s %s', 'dimax' ), $args['labels']['year'], get_the_date( 'Y' ) )
			);
		} // Archive
		else {
			$items[] = sprintf(
