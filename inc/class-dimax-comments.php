...
class Comments {
	...
	public function __construct() {
		add_action( 'dimax_after_close_post_content', array( $this, 'get_comment' ), 20 );

		add_action( 'dimax_after_open_comments_content', array( $this, 'get_title' ), 10 );
		add_action( 'dimax_after_open_comments_content', array( $this, 'comment_content' ), 20 );
		add_action( 'dimax_after_open_comments_content', 'paginate_comments_links', 30 );
		add_action( 'dimax_after_open_comments_content', array( $this, 'comment_fields' ), 40 );
	...
	public function get_title() {
		...
		printf( // WPCS: XSS OK.
			esc_html( _nx( 'Comment (%1$s)', 'Comments (%1$s)', $comments_number, 'comments title', 'dimax' ) ),
	...
	public function comment_fields() {
		if ( ! comments_open() ) {
			echo '<p class="no-comments">' . esc_html__( 'Comments are closed.', 'dimax' ) . '</p>';
		} else {
			$comment_field = '<p class="comment-form-comment"><textarea required id="comment" placeholder="' . esc_attr__( 'Content', 'dimax' ) . '" name="comment" cols="45" rows="7" aria-required="true"></textarea></p>';
			comment_form(
				array(
					'format'        => 'xhtml',
	...
	public function comment_content() {
		...
		wp_list_comments( array(
			...
			'short_ping'  => true,
			'style'       => 'ol',
			'callback'    => array( $this, 'dimax_comment' )
		) );
	...
	public function comment_form_fields() {
		...
		$fields = array(
			'author' => '<p class="comment-form-author">' .
			            '<input id ="author" placeholder="' . esc_attr__( 'Name', 'dimax' ) . ' " name="author" type="text" required value="' . esc_attr( $comment_author ) .
			            '" size    ="30"' . $aria_req . ' /></p>',

			'email' => '<p class="comment-form-email">' .
			           '<input id ="email" placeholder="' . esc_attr__( 'Email', 'dimax' ) . '" name="email" type="email" required value="' . esc_attr( $comment_author_email ) .
			           '" size    ="30"' . $aria_req . ' /></p>',

			'url' => '<p class="comment-form-url">' .
			         '<input id ="url" placeholder="' . esc_attr__( 'Website', 'dimax' ) . '" name="url" type="text" value="' . esc_attr( $comment_author_url ) .
			         '" size    ="30" /></p>'
		);
	 ...
	 * @return string
	 */
	public function dimax_comment( $comment, $args, $depth ) {
	...
	public function comment_template( $comments ) {
		...
		$date      = sprintf( esc_html__( '%1$s at %2$s', 'dimax' ), $comments['comment_date'], $comments['comment_time'] );
		$output[]  = sprintf( '<a href="%s" class="date">%s</a>', esc_url( $comments['comment_link'] ), $date );
		$output[]  = '</div>';
		if ( $comments['comment_approved'] == '0' ) {
			$output[] = sprintf( '<em class="comment-awaiting-moderation">%s</em>', esc_html__( 'Your comment is awaiting moderation.', 'dimax' ) );
		} else {
		...
		if ( current_user_can( 'edit_comment', $comments['comment_ID'] ) ) {
			$output[] = sprintf( '<a class="comment-edit-link" href="%s">%s</a>', esc_url( admin_url( 'comment.php?action=editcomment&amp;c=' ) . $comments['comment_ID'] ), esc_html__( 'Edit', 'dimax' ) );
		}
