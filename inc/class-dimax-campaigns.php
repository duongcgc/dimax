...
class Campaigns {
	...
	public function __construct() {
		if (  apply_filters( 'dimax_get_campaign_bar_position', Helper::get_option( 'campaign_bar_position' )) == 'after' ) {
			add_action( 'dimax_after_close_site_header', array( $this, 'display_campaign_bar' ), 10 );
		} else {
			add_action( 'dimax_before_open_site_header', array( $this, 'display_campaign_bar' ), 20 );
		}
	...
	public function display_campaign_bar() {
		if ( ! apply_filters( 'dimax_get_campaign_bar', Helper::get_option( 'campaign_bar' ) ) ) {
			return;
		}
