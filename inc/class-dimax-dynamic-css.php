...
class Dynamic_CSS {
	...
	public function __construct() {
		add_action( 'dimax_after_enqueue_style', array( $this, 'add_static_css' ) );
	}
	...
	public function add_static_css() {
		...
		wp_add_inline_style( 'dimax', apply_filters( 'dimax_inline_style', $parse_css ) );
	}
	...
	protected function header_static_css() {
		...
		if ( Helper::get_option( 'header_search_style' ) != 'icon' && intval( Helper::get_option( 'header_search_custom_color' ) ) ) {
			...
			if ( $header_search_button_color ) {
				...
				$static_css .= '#site-header .header-search.ra-search-form .search-submit .dimax-svg-icon, .site-header .header-search.form-type-full-width .search-submit .dimax-svg-icon { color: ' . esc_attr( $header_search_button_color ) . '}';
			}
	...
	protected function page_header_static_css() {
		...
		if ( $padding_top ) {
			$static_css .= '#page-header .page-header__title { padding-top: ' . $padding_top . 'px; }';
		}

		if ( $padding_bottom ) {
			$static_css .= '#page-header .page-header__title { padding-bottom: ' . $padding_bottom . 'px; }';
		}
	...
	public function page_boxed_static_css() {
		...
		if ( ! empty( $boxed_style ) ) {
			$static_css .= '.dimax-boxed-layout ' . ' {' . implode( '', $boxed_style ) . '}';
		}
