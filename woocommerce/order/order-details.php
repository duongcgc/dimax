...
$item_text = esc_html__('Order items', 'dimax');

if ( $item_count == 1 ) {
	$item_text = esc_html__('Order item', 'dimax');
}
?>
		...
		<thead>
			<tr>
				<th class="woocommerce-orders-table__cell order-number">
					...
					<span><?php echo esc_html( _x( '#', 'hash before order number', 'dimax' ) . $order->get_order_number() ); ?></span>
				</th>
				...
				<th class="woocommerce-orders-table__cell order-total">
					...
					<span><?php echo wp_kses_post( sprintf( _n( '%1$s', '%1$s', $item_count, 'dimax' ), $order->get_formatted_order_total() ) ); ?></span>
				</th>
				<th class="woocommerce-orders-table__cell order-method">
					<div class="order-title"><?php esc_html_e('Payment method:', 'dimax') ?></div>
...
<section class="woocommerce-order-total">
	<h2 class="woocommerce-order-downloads__title"><?php esc_html_e('Order Total', 'dimax') ?></h2>
	...
	<table class="woocommerce-table woocommerce-table--order-details shop_table order_details order_total">
		<tbody>
			...
			<?php if ( $order->get_customer_note() ) : ?>
				<tr>
					<th><?php esc_html_e( 'Note:', 'dimax' ); ?></th>
